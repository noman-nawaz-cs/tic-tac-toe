...
function checkRows(grid, player) {
    for(let i = 0; i < grid.length; i++){
        if (grid[i].every((item)=>item === player)) return true;
    }
    return false;
}
function checkColumns(grid, player) {
    ...
    for(let i = 0; i < grid.length; i++){
        flag = true;
        for(let j = 0; j < grid[i].length; j++)if (grid[j][i] !== player) flag = false;
        if (flag) return true;
    }
    return false;
}
function checkDiagonals(grid, player) {
    let flag = true;
    for(let i = 0; i < grid.length; i++)if (grid[i][i] !== player) flag = false;
    if (flag) return true;
    for(let i = 0; i < grid.length; i++){
        if (grid[i][grid.length - i - 1] !== player) return false;
    }
    return true;
}
...
function startGame(currentPlayer) {
    ...
    for(let i = 0; i < grid.length; i++)grid[i].addEventListener("click", ()=>{
...
function resetGrid() {
    ...
    for(let i = 0; i < gridCells.length; i++)gridCells[i].textContent = "";
    gridArray = new Array(gridSize).fill(null).map(()=>new Array(gridSize).fill(null));
...
function setActiveButton(button) {
    if (currentActiveButton) currentActiveButton.style.backgroundColor = "";
...
btn1 === null || btn1 === void 0 || btn1.addEventListener("click", ()=>{
    setActiveButton(btn1);
    const grid = document.getElementById("grid");
    resizeGrid(grid, 3);
});
btn2 === null || btn2 === void 0 || btn2.addEventListener("click", ()=>{
    setActiveButton(btn2);
    const grid = document.getElementById("grid");
    resizeGrid(grid, 5);
});
btn3 === null || btn3 === void 0 || btn3.addEventListener("click", ()=>{
    setActiveButton(btn3);
    const grid = document.getElementById("grid");
    resizeGrid(grid, 7);
});
playerX_btn === null || playerX_btn === void 0 || playerX_btn.addEventListener("click", ()=>{
    startGame("X");
});
playerO_btn === null || playerO_btn === void 0 || playerO_btn.addEventListener("click", ()=>{
    startGame("O");
});
resetButton === null || resetButton === void 0 || resetButton.addEventListener("click", ()=>{
    resetGrid();
});
